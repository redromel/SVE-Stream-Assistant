...
export default function App() {
  ...
  const handleReset = useCallback(() => {
    setState((s) => {
      const keep = (p: Player) => ({
        name: p.name,
        playerClassUniverse: p.playerClassUniverse,
        pronouns: p.pronouns,
  ...
  const handleSwapFirst = useCallback(() => {
    setState((s) => {
      ...
      players[newFirst] = {
        ...players[newFirst],
        evoPoints: s.players[newFirst === 0 ? 1 : 0].evoPoints,
        maxEvoPoints: s.players[newFirst === 0 ? 1 : 0].maxEvoPoints,
        pp: s.players[newFirst === 0 ? 1 : 0].pp,
        maxPP: s.players[newFirst === 0 ? 1 : 0].maxPP,
      };
      players[newFirst === 0 ? 1 : 0] = {
        ...players[newFirst === 0 ? 1 : 0],
        evoPoints: s.players[newFirst].evoPoints,
        maxEvoPoints: s.players[newFirst].maxEvoPoints,
        pp: s.players[newFirst].pp,
        maxPP: s.players[newFirst].maxPP,
      };
    ...
    >
      <header className="border-b border-border px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1
            className="text-sm font-bold tracking-widest uppercase text-foreground/80"
      ...
      <footer className="border-t border-border px-6 py-2 flex items-center justify-between">
        ...
        <span className="text-[10px] text-muted-foreground tracking-widest uppercase">
          Turn {state.turn} · {state.players[state.activePlayer].name}&apos;s
          turn
        </span>
      </footer>
    </div>
